...
interface WebDriver : Closeable {
     ...
     *
     * @return A [ai.platon.pulsar.driver.chrome.dom.model.NanoDOMTree] representing the current page state, or null if retrieval fails.
     * */
    @MCP
    @Throws(WebDriverException::class)
    suspend fun nanoDOMTree(): ai.platon.browser4.api.model.NanoDOMTree?

    @Throws(WebDriverException::class)
    suspend fun browserUseState(
        target: ai.platon.browser4.api.model.PageTarget = ai.platon.browser4.api.model.PageTarget(),
        snapshotOptions: ai.platon.browser4.api.model.SnapshotOptions = ai.platon.browser4.api.model.SnapshotOptions()
    ): BrowserUseState
