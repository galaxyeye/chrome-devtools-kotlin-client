        ...
        rpc.invokeOnElement(selector, actionName, scrollIntoView = true) { node ->
            withNodeObjectId(browserProtocol, node) { objectId ->
                val result = browserProtocol.callFunctionOn(
                    ...
                    objectId = objectId,
                    arguments = listOf(CallArgument(value = JsonPrimitive(shouldCheck))),
                    returnByValue = true,
                    userGesture = true,
                    awaitPromise = true
                )
        ...
        val result = rpc.invokeOnElement(selector, "selectOption") { node ->
            withNodeObjectId(browserProtocol, node) { objectId ->
                val res = browserProtocol.callFunctionOn(
                    functionDeclaration,
                    objectId = objectId,
                    arguments = listOf(CallArgument(value = JsonPrimitive(jsonValues))),
                    returnByValue = true
                )
            ...
            if (isolatedWorldJs.isNotBlank()) {
                ...
                if (evaluate.result.value?.jsonPrimitive?.content != "function") {
                    logger.warn(
                        ...
                        evaluate.result.value?.jsonPrimitive?.content
                    )
                }
            }
